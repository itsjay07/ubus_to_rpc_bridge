...
static struct ubus_method bridge_methods[] = {
    {
        .name = "welcome",
        .handler = handle_bridge_welcome
    },
};
    ...
    while (keep_running) {
        ...
        if (activity > 0 && FD_ISSET(listen_sock, &readfds)) {
            ...
            if (strcmp(req.method, "greet.welcome") == 0) {
                // For now, just echo back - we'll fix the ubus call later
                json_object* response = build_rpc_response(req.id, 
                    "Hello from bridge (simulated)");
                send_json(client_fd, response);
                json_object_put(response);
                log_info("RPC -> ubus: sent response");
            } else {
